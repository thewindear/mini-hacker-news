      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 uppercase flex-nowrap overflow-hidden">
          ...
          <div className="flex items-center gap-1 flex-shrink-0 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
            ...
            {item.url && (
              <button onClick={handleLinkClick} className="bg-gray-100 text-gray-500 lg:bg-transparent lg:text-gray-300 hover:text-blue-500 px-1 rounded text-[8px] font-black transition-all">LINK</button>
            )}
          </div>
          {item.descendants !== undefined && (
            <span className="ml-auto flex items-center gap-0.5 flex-shrink-0 text-gray-400">💬 {item.descendants}</span>
          )}
          {isNavigating && (
...
const UserDetail: React.FC<UserDetailProps> = ({ username, onClose, onStorySelect, hasActiveStory, favoriteIds, onToggleFavorite }) => {
  ...
  const handleItemClick = async (item: HNItem) => {
    if (item.type === 'story') {
      ...
      try {
        ...
        if (fullStory) onStorySelect(fullStory);
      } catch (err) {
    ...
    <div className="flex flex-col h-full bg-white animate-in fade-in slide-in-from-bottom duration-300 overflow-hidden">
      <header className="flex-shrink-0 px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white/90 backdrop-blur-md sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <button onClick={onClose} className="p-2 -ml-2 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-900 transition-all" title="Close profile">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          </button>
          <div><h2 className="text-[10px] font-black text-gray-400 tracking-widest uppercase">Member Profile</h2></div>
        </div>
        {hasActiveStory && (
          <button onClick={onClose} className="text-[10px] font-black uppercase tracking-widest bg-orange-500 text-white px-3 py-1.5 rounded-lg shadow-lg shadow-orange-100 hover:scale-105 active:scale-95 transition-all">Back</button>
        )}
      </header>
        ...
        <div className="max-w-5xl mx-auto px-6 py-6 sm:py-10">
          ...
          {user?.about && (
            <div className="mb-6">
              ...
              <div className="bg-gray-50/50 border border-gray-100 p-6 sm:p-8 rounded-[2rem] text-[15px] leading-relaxed text-gray-700 prose-comment shadow-sm" dangerouslySetInnerHTML={{ __html: user.about }} />
            </div>
          )}
          <div className="sticky top-0 bg-white z-10 pt-2 pb-4 mb-2 flex gap-1 border-b border-gray-50 overflow-x-auto scrollbar-hide">
            <button onClick={() => setActiveTab('stories')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'stories' ? 'bg-orange-500 text-white shadow-lg shadow-orange-100' : 'text-gray-400 hover:bg-gray-50'}`}>Submissions</button>
            <button onClick={() => setActiveTab('comments')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'comments' ? 'bg-orange-500 text-white shadow-lg shadow-orange-100' : 'text-gray-400 hover:bg-gray-50'}`}>Comments</button>
          </div>
          <div className="divide-y divide-gray-50 pb-12">
            {loading ? (
              [...Array(5)].map((_, i) => (
                <div key={i} className="flex gap-4 p-5 animate-pulse">
                  <div className="w-8 h-4 bg-gray-100 rounded" /><div className="flex-1 space-y-3"><div className="h-4 bg-gray-100 rounded w-full" /><div className="h-3 bg-gray-100 rounded w-1/3" /></div>
                </div>
              ))
            ) : items.length > 0 ? (
              items.map((item, index) => (
                <UserActivityItem key={item.id} item={item} index={index} targetLanguage={'Chinese'} onSelect={handleItemClick} navigatingId={navigatingId} isFavorite={favoriteIds.includes(item.id)} onToggleFavorite={onToggleFavorite} />
              ))
            ) : (
              <div className="text-center py-20">
                <div className="text-4xl mb-4 opacity-20">📭</div><p className="text-gray-300 font-bold uppercase tracking-widest text-[10px]">No activity recorded</p>
              </div>
            )}
          </div>
        </div>
